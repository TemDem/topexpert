(() => {
    ...
    function spollers() {
        ...
        function initSpollers() {
            spollersArray.forEach((spollersBlock => {
                ...
                if (mediaQuery) {
                    const mediaQueryArray = mediaQuery.split(",");
                    if (mediaQueryArray.length === 2) {
                        const mediaType = mediaQueryArray[1].trim() || "max-width";
                        const mediaValue = mediaQueryArray[0].trim();
                        if (window.matchMedia(`(${mediaType}: ${mediaValue}px)`).matches) initSpollerBlock(spollersBlock);
                    }
                } else initSpollerBlock(spollersBlock);
            }));
        }
        function initSpollerBlock(spollersBlock) {
            spollersBlock.classList.add("_spoller-init");
            const detailsItems = spollersBlock.querySelectorAll("details");
            if (detailsItems.length) detailsItems.forEach((details => {
                const summary = details.querySelector("summary");
                if (summary) summary.addEventListener("click", (function(e) {
                    e.preventDefault();
                    toggleDetailsSpoller(details);
                }));
            }));
            const customSpollers = spollersBlock.querySelectorAll("[data-spoller]");
            if (customSpollers.length) customSpollers.forEach((spoller => {
                spoller.addEventListener("click", (function() {
                    toggleCustomSpoller(spoller);
                }));
            }));
        }
        function toggleDetailsSpoller(details) {
            if (details.hasAttribute("open")) {
                details.removeAttribute("open");
                details.classList.remove("_spoller-open");
                const body = details.querySelector(".spollers__body") || Array.from(details.children).find((el => el !== details.querySelector("summary")));
                if (body) _slideUp(body, 300);
            } else {
                details.setAttribute("open", "open");
                details.classList.add("_spoller-open");
                const body = details.querySelector(".spollers__body") || Array.from(details.children).find((el => el !== details.querySelector("summary")));
                if (body) _slideDown(body, 300);
                if (details.closest("[data-one-spoller]")) {
                    const parent = details.closest("[data-spollers]");
                    ...
                    allDetails.forEach((item => {
                        if (item !== details && item.hasAttribute("open")) {
                            ...
                            const itemBody = item.querySelector(".spollers__body") || Array.from(item.children).find((el => el !== item.querySelector("summary")));
                            if (itemBody) _slideUp(itemBody, 300);
                        }
                    }));
                }
            }
        }
        function toggleCustomSpoller(spoller) {
            const parent = spoller.closest("[data-spollers]");
            const content = spoller.nextElementSibling;
            if (!content) return;
            if (spoller.classList.contains("_spoller-active")) {
                spoller.classList.remove("_spoller-active");
                _slideUp(content, 300);
            } else {
                spoller.classList.add("_spoller-active");
                _slideDown(content, 300);
                if (parent.hasAttribute("data-one-spoller")) {
                    const allActiveSpollers = parent.querySelectorAll("._spoller-active");
                    allActiveSpollers.forEach((item => {
                        if (item !== spoller) {
                            item.classList.remove("_spoller-active");
                            const itemContent = item.nextElementSibling;
                            if (itemContent) _slideUp(itemContent, 300);
                        }
                    }));
                }
            }
        ...
        initSpollers();
        window.addEventListener("resize", initSpollers);
    }
    function menuInit() {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".team__slider")) new swiper_core_Swiper(".team__slider", {
            ...
            navigation: {
                prevEl: ".team__arr.arr_prev",
                nextEl: ".team__arr.arr_next"
            },
            on: {}
        });
        if (document.querySelector(".projects__slider")) new swiper_core_Swiper(".projects__slider", {
            ...
            navigation: {
                prevEl: ".projects__arr.arr_prev",
                nextEl: ".projects__arr.arr_next"
            },
            on: {}
        });
        if (document.querySelector(".project__slider") && document.querySelector(".project__thumb")) document.querySelectorAll(".project__slider-wrap").forEach((wrap => {
            const thumbs = new swiper_core_Swiper(wrap.querySelector(".project__thumb"), {
                direction: "vertical",
                observer: true,
                observeParents: true,
                slidesPerView: 3,
            ...
            new swiper_core_Swiper(wrap.querySelector(".project__slider"), {
                modules: [ Thumb ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
        ...
        if (document.querySelector(".reviews-services__slider")) new swiper_core_Swiper(".reviews-services__slider", {
            modules: [ Pagination, Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
            ...
            pagination: {
                el: ".reviews-services__pagination",
                clickable: true
            },
            navigation: {
                prevEl: ".reviews-services__arr_prev",
                nextEl: ".reviews-services__arr_next"
            },
            on: {}
        });
    }
    ...
    function sortSteps(e) {
        if (e.matches) {
            ...
            items.sort(((a, b) => {
                const numA = parseInt(a.querySelector(".items-steps__num").textContent, 10);
                const numB = parseInt(b.querySelector(".items-steps__num").textContent, 10);
                return numA - numB;
            }));
            items.forEach((item => stepsContainer.appendChild(item)));
